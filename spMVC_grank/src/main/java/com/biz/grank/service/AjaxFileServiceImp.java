...
//		fDao.deletes(bno);
	...
	public void insert(BoardDto bDto) {
		...
		for(String file_name : board_files) {
			int bno = (int) bDto.getBno();
			String origin_name = file_name.substring(49);
			String path = file_name.substring(0,12);
			String name = file_name.substring(12);
			String thumbnail = path + "s_" + name;
			int index = file_name.lastIndexOf(".");
			String file_type = file_name.substring(index+1);
			FileDto fDto = FileDto.builder()
							.file_name(file_name)
							.thumbnail(thumbnail)
							.origin_name(origin_name)
							.format_name(file_type.toUpperCase())
							.bno(bno).build();
			fDao.insert(fDto);
		}
	}
